...
train_val_symbols = symbols[:14]
test_symbols = symbols[14:]
...
def breakout_identify(df):
    ...
    # Check for bullish breakout: close of the latest next 5 > high of the last of last 4
    if latest_close_of_next_5 > last_high_of_last_4 and last_close_of_last_4 > resistance:
        return "bullish_breakout"

    # Check for bearish breakout: close of the latest next 5 < low of the last of last 4
...
def adjust_no_breakout_folder(breakout_dir):
    ...
    # Determine the target number of files for "no_breakout"
    target_count = max(bullish_count, bearish_count)

    # If "no_breakout" folder has excess files, delete them randomly
...
start_date = datetime.now() - timedelta(days=365 * 1) # Time range: last 1 years
end_date = datetime.now()

# Process training/validation and testing symbols
for symbol_group, train_val_dir, test_dir in [
    (train_val_symbols, output_train_val_dir, output_test_dir),
    (test_symbols, output_train_val_dir, output_test_dir),
]:
